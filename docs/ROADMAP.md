...
- [ ] **Verify APRS Mic-E against a real captured packet.** Now the top of
      the queue -- P1's remaining items are all APRS/decode-path
      verification, not connected-mode. APRS UI frames are already visible
      passing through the monitor from real stations, but Mic-E specifically
      has not been exercised yet. The Mic-E fixtures

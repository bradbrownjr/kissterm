...
| Radiogram-ICS213 layout | The RRI 2026 guidelines above (pages 3-4), and Winlink's "RRI ICS213 Radiogram" template v4.7 in Winlink Standard Forms 1.1.20.0 (see "Message forms: sources") | HXI, the signer's position after the name, the subject line after the signature. The guidelines' traffic-net order is used (signature and subject after the text); Winlink's template uses the delivery order (before it). |
...
| Packet BBS upload (`ST`, title, body) | [ARRL NTS MPG chapter 6, *NTS Digital*, 6.2.1 (2002)](https://www.arrl.org/files/file/Public%20Service/MPG604A.pdf) | `ST` routing, and the 30-character title limit. Its `NR` before the number, blank-line separators and `QTC` title are superseded (see below). |

...
COMMANDS: tuple[Command, ...] = (
    ...
    Command("disconnect", "Disconnect", "Session", "D",
            "End the session on the active Terminal tab, or cancel a connect "
            "that is still trying", key="ctrl+d",
    ...
    Command("toggle_contacts", "Address book", "View", "B",
            ...
            tabs=("terminal", "mail", "bulletins", "files"),
            footer=("terminal", "mail", "bulletins", "files"), short="Book"),
...
#: Markers around README.md's generated key table. `scripts/sync_docs.py`
#: rewrites what is between them; `tests/unit/test_docs_keys.py` fails if it
#: differs from `markdown_key_table()`.
README_KEYS_START = "<!-- keys:start (generated by scripts/sync_docs.py from kissterm/ui/commands.py; do not edit) -->"
README_KEYS_END = "<!-- keys:end -->"


def _tabs_note(command: Command) -> str:
    return ", ".join(TAB_TITLES[t] for t in command.tabs)


def markdown_key_table() -> str:
    """README's key table, from `COMMANDS` -- the sixth thing generated from
    the one table, for the same reason as the other five: the hand-written
    copy had already drifted (it sent operators to Settings on F5, which is
    Monitor). One row per bound key, in the order an operator meets them:
    F1, the tab keys (sharing a row), F10, then the Ctrl keys in Footer
    order. A key that does different things on different tabs says so.
    """
    tab_keys = [c for c in COMMANDS if action_base(c.action) == "show_tab"]
    by_key: dict[str, list[Command]] = {}
    for command in COMMANDS:
        if command.key and action_base(command.action) != "show_tab":
            by_key.setdefault(command.key, []).append(command)
    ...
    def rank(key: str) -> tuple[int, int]:
        ...
        if key == "f10":
            return (2, 0)
        actions = [action_base(c.action) for c in by_key[key]]
        for i, action in enumerate(FOOTER_ORDER):
            if action in actions:
                return (3, i)
        return (4, 0)

    def cell(key: str) -> str:
        return f"`{key_label(key, short=False)}`"

    def describe(command: Command) -> str:
        return f"**{command.label}** -- {command.help}"

    rows: list[tuple[tuple[int, int], str, str]] = [(
        (1, 0),
        " ".join(cell(c.key) for c in tab_keys),
        "Show the " + " / ".join(c.label for c in tab_keys)
        + " tab. Each tab's label starts with its key",
    )]
    for key, entries in by_key.items():
        if len(entries) == 1:
            text = describe(entries[0])
            if entries[0].tabs:
                text += f" ({_tabs_note(entries[0])})"
        else:
            entries = sorted(entries, key=lambda c: TAB_ORDER.index(c.tabs[0]))
            text = "; ".join(f"{_tabs_note(c)}: {describe(c)}" for c in entries)
        rows.append((rank(key), cell(key), text))
    rows.sort(key=lambda row: row[0])
    lines = ["| Key | What it does |", "|-----|--------------|"]
    lines += [f"| {key} | {text} |" for _, key, text in rows]
    return "\n".join(lines)

...
# ---------------------------------------------------------------------------
# Address Book -- the same list the Connect dialog's history offers,
# editable here without attempting a live connect.
# ---------------------------------------------------------------------------


async def _addressbook_tab(app, pilot, tmp_path):
    app.addressbook = AddressBook(tmp_path / "addressbook.json")
    await _settings_tab(app, pilot)
    app.query_one("#settings-tabs", TabbedContent).active = "settings-tab-address-book"
    await pilot.pause()
...
async def test_every_saved_station_appears_in_the_table(tmp_path):
    ...
    async with app.run_test(size=(120, 44)) as pilot:
        app.addressbook = AddressBook(tmp_path / "addressbook.json")
        app.addressbook.record_attempt("WS1EC-7", script="CLYDE")
        app.addressbook.record_attempt("W1LH-6", hops="N1QFY", credential="Personal BBS login")
        await _settings_tab(app, pilot)
        app.query_one("#settings-tabs", TabbedContent).active = "settings-tab-address-book"
        await pilot.pause()

        from kissterm.ui.settings_pane import _AddressBookTable

        table = app.query_one("#settings-addressbook-table", _AddressBookTable)
        targets = [str(table.get_cell_at((r, 0))) for r in range(table.row_count)]
        assert set(targets) == {"WS1EC-7", "W1LH-6"}
    station.close()


@pytest.mark.asyncio
async def test_new_addressbook_entry_via_the_dialog(tmp_path):
    app, station = await _app(Config(mycall=str(MYCALL)))
    async with app.run_test(size=(120, 44)) as pilot:
        await _addressbook_tab(app, pilot, tmp_path)

        app.query_one(SettingsPane)._addressbook_new()
        await pilot.pause()
        await asyncio.sleep(0.05)

        from kissterm.ui.dialogs import AddressBookEntryScreen, ConnectRequest

        assert isinstance(app.screen, AddressBookEntryScreen), type(app.screen).__name__
        await app.screen.dismiss(ConnectRequest("W1LH-6", hops="N1QFY, AB1KI-15"))
        await pilot.pause()

        entry = app.addressbook.entries[0]
        assert entry.target == "W1LH-6"
        assert entry.hops == "N1QFY, AB1KI-15"
        assert entry.attempts == 0, "creating an entry in Settings is not an attempt to reach it"
    station.close()


@pytest.mark.asyncio
async def test_editing_an_addressbook_entry_preserves_its_counters(tmp_path):
    app, station = await _app(Config(mycall=str(MYCALL)))
    async with app.run_test(size=(120, 44)) as pilot:
        await _addressbook_tab(app, pilot, tmp_path)
        app.addressbook.record_attempt("WS1EC-7")
        app.addressbook.record_connect("WS1EC-7")
        app.query_one(SettingsPane)._render_addressbook()
        await pilot.pause()
        ...
        app.query_one(SettingsPane)._addressbook_edit_selected()
        await pilot.pause()
        await asyncio.sleep(0.05)
        ...
        assert isinstance(app.screen, AddressBookEntryScreen), type(app.screen).__name__
        await app.screen.dismiss(ConnectRequest("WS1EC-7", script="NEWSCRIPT"))
        await pilot.pause()
...
@pytest.mark.asyncio
async def test_forgetting_an_addressbook_entry(tmp_path):
    app, station = await _app(Config(mycall=str(MYCALL)))
    async with app.run_test(size=(120, 44)) as pilot:
        await _addressbook_tab(app, pilot, tmp_path)
        app.addressbook.record_attempt("Keep me")
        app.addressbook.record_attempt("Drop me")
        app.query_one(SettingsPane)._render_addressbook()
        await pilot.pause()
        ...
        table = app.query_one("#settings-addressbook-table", _AddressBookTable)
        table.move_cursor(row=0)  # "Drop me" -- most recently touched, at the front
        await pilot.pause()

        app.query_one(SettingsPane)._addressbook_forget_selected()
        await pilot.pause()

        assert [e.target for e in app.addressbook.entries] == ["Keep me"]
    station.close()


@pytest.mark.asyncio
async def test_insert_f2_and_delete_act_only_while_the_table_is_focused(tmp_path):
    """The whole point of binding these on the table itself, not the pane:
    Delete must still delete a character out of an Input on another tab,
    not forget an address-book row nobody was even looking at."""
    app, station = await _app(Config(mycall=str(MYCALL)))
    async with app.run_test(size=(120, 44)) as pilot:
        await _addressbook_tab(app, pilot, tmp_path)
        app.addressbook.record_attempt("WS1EC-7")
        ...
        app.query_one("#settings-tabs", TabbedContent).active = "settings-tab-station"
        await pilot.pause()
        app.query_one("#set-mycall", Input).focus()
        await pilot.pause()

...
def test_mic_e_northern_western_hemisphere():
    # Destination "42CQ3P" and info generated by this module's own Mic-E
    # inverse encoder for: lat=42 21.30' N, lon=071 03.50' W, course=45,
    # speed=30kt, message bits (0,0,1) -> "Priority", symbol '/'+'>'  (car).
    #
    # The expected message text here was "En Route" until 2026-09-08: this
    # fixture was hand-built against _MICE_MESSAGES, and that table shipped
    # bit-inverted (see kissterm/aprs/mice.py's module docstring) -- so the
    # fixture and the bug agreed with each other and this test could not
    # catch it. Only cross-checking real captured traffic against an
    # independent decoder (aprslib) surfaced it. The destination bytes
    # (and therefore the actual bits under test, (0,0,1)) were unchanged
    # at that point; only the expected text was wrong.
    #
    # The destination itself changed right after, same date: positions 4
    # and 6 (the N/S and E/W flags) were "B" and "A" -- A-K letters -- which
    # this codebase's own bit rule read as "flag set" (letter = 1) same as
    # the message bits. Real N/S/offset/E-W flags are only set by the
    # CUSTOM-range letters (P-Y/Z); A-K never legitimately appears there
    # (see kissterm/aprs/mice.py's module docstring). "B"/"A" happened to
    # still decode North/West under the old rule by coincidence of which
    # digits they represent, so this fixture could not have caught that bug
    # either -- "Q"/"P" (custom-range letters for the same digits) are what
    # a real encoder would actually send.
    frame = _ui_frame("42CQ3P", "N1ABC-9", b"`c\x1fN\x1f\x1cI>/Mobile")
    pkt = parse_packet(frame)
    assert pkt is not None
    ...
    assert pos.symbol_table == "/"
    assert pos.symbol_code == ">"
    assert pos.mic_e_message == "Priority"
    assert pos.comment == "Mobile"
...
def test_mic_e_southern_eastern_hemisphere():
    ...
    # "Off Duty" (corrected 2026-09-08 -- see test above and
    # kissterm/aprs/mice.py's module docstring; this used to assert
    # "Emergency" under the same then-wrong table).
    ...
    assert pos.mic_e_message == "Off Duty"


def test_mic_e_emergency_message():
    # Same fixture as the northern/western test above, but with destination
    # position 3 changed from the letter 'C' (digit "2", message bit 1) to
    # the plain digit '2' (same digit, message bit 0) -- "422Q3P" instead of
    # "42CQ3P". That flips message bits to (0,0,0) -> "Emergency" without
    # touching the latitude value those positions also encode (C and '2'
    # both represent digit "2" -- see _MICE_TABLE), so lat/lon/course/speed
    # are identical to the fixture above. Added 2026-09-08 specifically to
    # keep the Emergency case -- the one entry that must never be confused
    # with a routine status -- under direct test after _MICE_MESSAGES's
    # inverted-table bug was found and fixed.
    frame = _ui_frame("422Q3P", "N1ABC-9", b"`c\x1fN\x1f\x1cI>/Mobile")
    pkt = parse_packet(frame)
    assert pkt is not None
    assert pkt.kind == "mic-e"
    pos = pkt.data
...
def test_mic_e_old_data_type():
    ...
    frame = _ui_frame("42CQ3P", "N1ABC-9", b"'c\x1fN\x1f\x1cI>/")
    pkt = parse_packet(frame)
    assert pkt.kind == "mic-e"
    assert pkt.data.mic_e_message == "Priority"

...
def test_mic_e_northern_western_hemisphere():
    ...
    # (and therefore the actual bits under test, (0,0,1)) are unchanged;
    # only the expected text was wrong.
...
def test_mic_e_emergency_message():
    ...
    # the plain digit '2' (same digit, message bit 0) -- "422B3A" instead of
    # "42CB3A". That flips message bits to (0,0,0) -> "Emergency" without
    ...
    frame = _ui_frame("422B3A", "N1ABC-9", b"`c\x1fN\x1f\x1cI>/Mobile")
    pkt = parse_packet(frame)
    assert pkt is not None
    assert pkt.kind == "mic-e"
    pos = pkt.data
